...
# assuming location of (x_backward_limit_switch, y_backward_limit_switch) is (0, 0)
def translate_real_world_to_encoder_world(x1, y1, x2, y2, x_bkd_lim, y_bkd_lim):
    x_world = x2 - x1
    x_steps = distance_translate_real_world_to_encoder_world(x_world) - x_bkd_lim
    y_world = y2 - y1
    y_steps = distance_translate_real_world_to_encoder_world(y_world) - y_bkd_lim
    return x_steps, y_steps

# stats for this function acquired from motor and encoder datasheet
def distance_translate_real_world_to_encoder_world(distance_in_cm: float):
...
async def move_xy(hardware, curr_location, new_location):
    ...
    moveMotor(hardware['X_MOTOR'], hardware['X_ENCODER'], location_diff[0])
    moveMotor(hardware['Y_MOTOR'], hardware['Y_ENCODER'], location_diff[1])
    return 0
...
# z axis is simply not real
# the proof of z axis movement is trivial and left to the reader as an exercise
...
def find_and_execute_path(new_location, hardware):
    ...
    for i in range(1, len(get_points)):
        move_xy(hardware, get_points[i - 1], get_points[i])
    # pick up piece
